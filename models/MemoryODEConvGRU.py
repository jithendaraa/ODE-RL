...
class ODEConvGRU(nn.Module):
    ...
    def forward(self, inputs, batch_dict):
        ...
        # ODE decoding: Given first_point_enc (z_0) and time_steps_to_predict([t_i,....t_(i+n)]), we predict sol_m([m_i,...m_(i+n)])
        sol_m = self.diffeq_solver(first_point_enc, time_steps_to_predict)
        # add memory to the initial point
        sol_y = sol_m + first_point_enc
