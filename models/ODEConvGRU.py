...
class ODEConvGRU(nn.Module):
    def __init__(self, opt, device, downsize=True):
        ...
        self.conv_encoder = utils.create_convnet(n_inputs=opt.in_channels, 
                                                 n_units=opt.conv_n_units).to(device)
        
        # Init encoder ODE with a convnet
        self.ode_encoder_func = ODEFunc(n_inputs=opt.conv_n_units[-1], 
        ...
        self.ode_decoder_func = ODEFunc(n_inputs=opt.neural_ode_n_units[-1], 
                                       n_units=opt.neural_ode_n_units,
                                       device=device)
        ...
        self.conv_decoder = utils.create_transpose_convnet(opt.neural_ode_decoder_out_ch, 
                                                           n_units=opt.deconv_n_units).to(device)
    ...
    def forward(self, inputs, batch_dict):
        ...
        if self.opt.memory:
            y = first_point_enc
            _,c0,h0,w0 = y.size()
            sol_y = torch.zeros(b,t,c0,h0,w0).to(self.device)
            for i in range(0, pred_t_len):
                t = time_steps_to_predict[i]
                m = self.diffeq_solver(y, t.unsqueeze(0))
                y1 = y + m[:,0,...]
                y = y1
                sol_y[:,i,...] = y
        else:    
    ...
    def get_loss(self, pred_frames, truth):
        ...
        loss = loss_function(pred_frames, truth)
        return loss
