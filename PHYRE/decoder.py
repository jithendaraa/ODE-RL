...
class Decoder_ODEModel(nn.Module):
    def __init__(self, ode_specs, lr=1e-3, device=None):
        super().__init__()
...
class Decoder(nn.Module):
    def __init__(self, subnets, convgrus, ode_specs, predict_timesteps, lr=1e-3, device=None):
        super().__init__()
        ...
        self.device = device
        self.ode_model = Decoder_ODEModel(ode_specs, device=self.device)
    ...
    def forward(self, h_s0):
        ...
        outputs = odeint(self.ode_model, h_s0, torch.tensor(self.predict_timesteps)).to(self.device)
        for i in list(range(1, self.subnet_blocks+1)):  outputs = self.forward_by_stage(outputs, getattr(self, 'deconv' + str(i)))
        return outputs
