...
class WorldModel(tools.Module):
  ...
  def __init__(self, step, config):
    self._step = step
    self._config = config
    if self._config.encoder_model == 'encoder':
      self.encoder = networks.ConvEncoder(config.cnn_depth, config.act, config.encoder_kernels)
    elif self._config.encoder_model == 'impala':
      print('Using IMPALA encoder')
      self.encoder = networks.ImpalaCNN(flatten_attn=config.flatten_attn, attention=config.dyn_attention, spatial=config.dyn_spatial)
    elif self._config.encoder_model in ['resnet18', 'resnet34', 'resnet50']:
      print("Using ResNet50 Encoder")
      self.encoder = networks.ResNetPretrained(version=self._config.encoder_model, img_size=config.size, shape=(config.dyn_spatial, config.dyn_spatial))
    else:
      raise NotImplementedError(self._config.encoder_model)
    self.dynamics = networks.RSSM(
        config.dyn_stoch, config.dyn_spatial, config.dyn_hidden, config.dyn_kernel,
        config.dyn_input_layers, config.dyn_output_layers,
        config.dyn_rec_depth, config.dyn_shared, config.dyn_discrete,
        config.act, config.dyn_skip, config.dyn_mean_act, config.dyn_std_act,
    ...
    self.heads['image'] = networks.ConvDecoder(
        ...
        config.decoder_thin, resnet_shape)
  ...
  def train(self, data):
    ...
    with tf.GradientTape() as model_tape:
      ...
      if self._config.encoder_model == 'encoder':
        embed = tools._convert_conv(embed, self.dynamics._spatial_size)
      ...
      # prior_prob = 0.3 #np.random.uniform(0.1, 0.6)
      if self._config.dyn_cell == 's_convgru':
        sparsity_loss = self.dynamics.sparsity_loss(post, self._config.dyn_gate_prior, self._config.dyn_gate_free, self._config.dyn_gate_scale, forward=True)
      else:
        sparsity_loss = 0
      # print ('printing prior', prior_prob)
      ...
      for name, head in self.heads.items():
        ...
        feat = tools._convert_dense(feat)
    ...
    if self._config.dyn_cell == 's_convgru':
      metrics['sparsity_c_mean'] = tf.reduce_mean(tf.reduce_sum(post['u_sample'], axis=-1))
      metrics['sparsity_t_mean'] = tf.reduce_mean(tf.reduce_sum(post['u_sample'], axis=-2))
      metrics['sparsity_c_std'] = tf.math.reduce_std(tf.reduce_sum(post['u_sample'], axis=-1))
      metrics['sparsity_t_std'] = tf.math.reduce_std(tf.reduce_sum(post['u_sample'], axis=-2))
  ...
  def preprocess(self, obs):
    ...
    if self._config.encoder_model in ['resnet18', 'resnet34', 'resnet50']:
      obs['resnet_image'] = tf.keras.applications.resnet50.preprocess_input(obs['obs'])
      obs['resnet_image'] = tf.cast(obs['resnet_image'], dtype)
    if 'discount' in obs:
  ...
  def video_pred(self, data, initial_frames=10, nenvs=6):
    ...
    recon = self.heads['image'](self.dynamics.get_feat(states, True)).mode()[:nenvs]
    init = {k: v[:, initial_frames-1] for k, v in states.items()}
    prior = self.dynamics.imagine((nenvs, total_frames-initial_frames), init)
    openl = self.heads['image'](self.dynamics.get_feat(prior, True)).mode()
...
class ConvGRUClassifier(tools.Module):
  def __init__(self, config, world_model, task, n_classes):
    ...
    self._spatial_size = config.dyn_spatial
    self._hidden_channels = config.dyn_hidden
    self._kernel = config.dyn_kernel
    self._attention_layer = None
    self._task = task
    self._cell_type = config.dyn_classifer_cell
    if config.dyn_classifer_cell == 'convgru':
      print("Using ConvGRU in the Classifier")
      self._cell = networks.ConvGRUCell(spatial=self._spatial_size, depth=self._hidden_channels, kernel=self._kernel, attention_layer=self._attention_layer)
    elif config.dyn_classifer_cell == 's_convgru':
      print("Using StochasticConvGRUCell in the Classifier")
      self._cell = networks.StochasticConvGRUCell(spatial=self._spatial_size, depth=self._hidden_channels, kernel=self._kernel, attention_layer=self._attention_layer)
    elif config.dyn_classifer_cell == 'gru':
  ...
  def reshape_data(self, data):
    ...
    data = tf.reshape(data, [B, nB // B] + list(data.shape[-3:]))
    ...
    if self._cell is not None:
      ...
      for post_state in post:
        if self._cell_type == 'gru':
          post_state = tools._convert_dense(post_state)
  ...
  def action_pred(self, data):
    ...
    if self._world_model._config.encoder_model == 'encoder':
        embed = tools._convert_conv(embed, self._world_model.dynamics._spatial_size)
...
#         # print ('imag_feat ', imag_feat.shape, imag_state.shape)
...
#       if not self._world_model.dynamics._discrete:
#         state_ent = tf.reduce_sum(state_ent, axis=-1)

#       print ('state_ent shape ', state_ent.shape)
...
#       value = self.value(tools._convert_dense(value_input), tf.float32)[:-1]
...
#     metrics['state_ent'] = tf.reduce_mean(state_ent)
...
#       # print ("after action ", inp.shape, action.shape)
#       succ = dynamics.img_step(state, tools._convert_conv(action, dynamics._spatial_size), sample=self._config.imag_sample)
...
#       # if self._config.reduce_state_ent == 'mean':
#       #   reshape_state_ent = tf.math.reduce_mean(state_ent[:-1], axis=-1)
#       # elif self._config.reduce_state_ent == 'sum':
#       #   reshape_state_ent = tf.math.reduce_sum(state_ent[:-1], axis=-1)
#       # else:
#       #   raise NotImplementedError(self._config.reduce_state_ent)
#       reward += self._config.actor_state_entropy() * state_ent
#     if slow:
#       value = self._slow_value(tools._convert_dense(imag_feat), tf.float32).mode()
#     else:
#       value = self.value(tools._convert_dense(imag_feat), tf.float32).mode()
...
#       # else:
#       #   raise NotImplementedError(self._config.reduce_state_ent)
#       actor_target += self._config.actor_state_entropy() * tf.cast(state_ent[:-1], dtype=actor_target.dtype)
