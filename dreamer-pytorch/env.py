...
class PhyreEnv():
  ...
  def get_done(self):
    if self.simulation is None or self.simulation.images is None or self.simulation_frame >= len(self.simulation.images): return True
    return False

  def step(self, action):
    action = (action.detach().numpy() + 1.0)/2.0
    # action = action.detach().numpy()
...
class EnvBatcher():
  ...
  def get_done(self):
    dones = [env.get_done() for env in self.envs]
    return dones
