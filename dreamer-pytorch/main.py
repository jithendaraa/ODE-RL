...
def arg_parser():
  ...
  parser.add_argument('--collect-interval', type=int, default=15, metavar='C', help='Collect interval')
  ...
  parser.add_argument('--model_learning-rate', type=float, default=6e-5, metavar='α', help='Learning rate') 
  parser.add_argument('--actor_learning-rate', type=float, default=6e-5, metavar='α', help='Learning rate') 
  parser.add_argument('--value_learning-rate', type=float, default=6e-5, metavar='α', help='Learning rate')
...
metrics = {'steps': [], 'episodes': [], 'train_rewards': [], 'test_episodes': [], 'test_rewards': [], 
           'observation_loss': [], 'reward_loss': [], 'kl_loss': [], 'actor_loss': [], 'value_loss': [], 'grad_norms': []}
...
if args.experience_replay is not '' and os.path.exists(args.experience_replay):
  ...
  for s in range(1, args.seed_episodes + 1):
    ...
    while not done:
      ...
      t += 1
      # if done: print("Seed epsiodes ended at ", t, done, action)
...
if args.algo=="dreamer":
  print("running dreamer...")
  planner = actor_model
else:
  planner = MPCPlanner(env.action_size, args.planning_horizon, args.optimisation_iters, args.candidates, args.top_candidates, transition_model, reward_model, phyre_env=phyre_env)
...
def update_belief_and_act(args, env, planner, transition_model, encoder, belief, posterior_state, action, observation, explore=False):
  ...
  if phyre_env:
    # Convert action from [-1, 1] range to [0, 1] for phyre
...
for episode in tqdm(range(metrics['episodes'][-1] + 1, args.episodes + 1), total=args.episodes, initial=metrics['episodes'][-1] + 1):
  ...
  with torch.no_grad():
    ...
    for t in pbar:
      ...
      if done:
        print()
        # print("Data collection:", done, t, action)
...
#     Set models to eval mode
    ...
    test_envs = EnvBatcher(Env, (args.env, args.symbolic_env, args.seed, args.max_episode_length, args.action_repeat, args.bit_depth, args.episodes, False if args.cross_class else True), {}, args.test_episodes)
    ...
    with torch.no_grad():
      ...
      for t in pbar:
        ...
        if not args.symbolic_env:  # Collect real vs. predicted frames for video
          # print("SHape 1: ", observation.size())
          r = observation_model(belief, posterior_state).cpu()
          # print("Shape 2: ", r.size())
    ...
    if not args.symbolic_env:
      ...
      writer.add_image('test_episode_' + str(episode_str), torch.as_tensor(video_frames[-1]))
...
# Plot gradient norm for each model
# Compare those curves with the curves that dreamer comes with
# Small seed ep, large collect interval to see how ObsModel does before the RL part kicks in
