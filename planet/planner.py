...
class MPCPlanner(jit.ScriptModule):
  ...
  def __init__(self, action_size, planning_horizon, optimisation_iters, candidates, top_candidates, transition_model, reward_model, min_action=-inf, max_action=inf, phyre_env=False):
    super().__init__()
    ...
    if phyre_env is True: self.optimisation_iters = 1
  ...
  def forward(self, belief, state):
    ...
    for _ in range(self.optimisation_iters):
      ...
      if self.phyre_env is True:
        actions = torch.rand(self.planning_horizon, B, self.candidates, self.action_size, device=belief.device).view(self.planning_horizon, B * self.candidates, self.action_size)
      else:
