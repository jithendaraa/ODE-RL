...
def init_dataset(seed_episodes, env, metrics, phyre_env, D):
...
if args.experience_replay is not '' and os.path.exists(args.experience_replay):
  ...
  D = ExperienceReplay(args.experience_size, args.symbolic_env, env.observation_size, env.action_size, args.bit_depth, args.device, args.env[:5]=='phyre')
  # Initialise dataset D with S random seed episodes
  env, metrics, D = init_dataset(args.seed_episodes, env, metrics, phyre_env, D)


# Section 2: Initialise model parameters randomly for Latent Space model
# Encoder to infer approximate belief over current hidden state
...
print("Env action range:", env.action_range[0], env.action_range[1])
planner = MPCPlanner(env.action_size, args.planning_horizon, args.optimisation_iters, args.candidates, args.top_candidates, transition_model, reward_model, env.action_range[0], env.action_range[1], phyre_env=phyre_env)
...
def update_belief_and_act(args, env, planner, transition_model, encoder, belief, posterior_state, action, observation, min_action=-inf, max_action=inf, explore=False):
  ...
  if explore: action = action + args.action_noise * torch.randn_like(action)  # Add exploration noise ε ~ p(ε) to the action
...
for episode in tqdm(range(metrics['episodes'][-1] + 1, args.episodes + 1), total=args.episodes, initial=metrics['episodes'][-1] + 1):
  ...
  for s in tqdm(range(args.collect_interval)):
    ...
    if args.global_kl_beta != 0:  kl_loss += args.global_kl_beta * kl_divergence(Normal(posterior_means, posterior_std_devs), global_prior).sum(dim=2).mean(dim=(0, 1))
